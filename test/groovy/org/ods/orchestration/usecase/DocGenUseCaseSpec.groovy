...
class DocGenUseCaseSpec extends SpecHelper {
    ...
    def setup() {
        ...
        usecase = Spy(new DocGenUseCaseImpl(context, steps, util, docGen, nexus, pdf, jenkins))
    }
    ...
    def "create document"() {
        ...
        def version = context.buildParams.version
        ...
        def basename = "${documentType}-${context.key}-${repo.id}-${version}-${steps.env.BUILD_ID}"
    ...
    def "create document without repo"() {
        ...
        // Argument Constraints
        def basename = "${documentType}-${context.key}-${version}-${steps.env.BUILD_ID}"
    ...
    def "create document with documentTypeEmbedded"() {
        ...
        // Argument Constraints
        def basename = "${documentTypeEmbedded}-${context.key}-${repo.id}-${version}-${steps.env.BUILD_ID}"
    ...
    def "create overall document removes previously stored repository-level documents"() {
        ...
        context.repositories.first().data.documents[documentType] = docName

        def path = "${this.steps.env.WORKSPACE}/reports/${context.repositories.first().id}"
        ...
        then:
        context.repositories.first().data.documents[documentType] == null
    }
    ...
    def "get document basename"() {
        ...
        then:
        result == "${documentType}-${context.key}-${version}-${build}"
    }
