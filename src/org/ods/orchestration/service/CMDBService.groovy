    ...
    private void loadRelationalCisData(Map parentNode, Closure ciRelationsLookupStrategy, Closure relationSanitizerStrategy, List parentNodeIds = [], int depth = 0, int maxDepth = MAX_DEPTH) {
        ...
        for (Map item in ciRelationsIds) {
            ...
            if (childSysClass) {
                ...
                if (childNode.name.contains("-IO-")) {
                  logger.debug "CMDB: IONode: ${childNode}"
                }
                ...
                if (childSysClass.sys_class_name =~ /(?i)ci_interface|ci_db_catalog|ci_database|ci_service/) {
                    ...
                    logger.debug "CMDB: No further relationship look-up configured for class ${childSysClass.sys_class_name} (name ${childNode.name})"
                }
            }
        }
    }
